...
class OptimalTransportProblem(ABC):
    def __init__(
        self, kwargs_source: dict, kwargs_destination: dict, n_samples: int
    ) -> None:
        self.n_samples = n_samples
        self.source_coordinates = [
            tuple(norm.rvs(**kwargs_source)) for i in range(0, self.n_samples)
        ]
        self.destination_coordinates = [
            tuple(norm.rvs(**kwargs_destination)) for i in range(0, self.n_samples)
        ]
...
class MILPSolver(OptimalTransportProblem):
    ...
    def solve(
        ...
        cost = (
            pd.DataFrame(self.cost_matrix)
            .stack()
            .reset_index()
            .set_index(["level_0", "level_1"])[0]
            .to_dict()
        )
        model.cost = pe.Param(model.source, model.destination, initialize=cost)

        model.y = pe.Var(
            model.source, model.destination, domain=decision_domain, initialize=0
